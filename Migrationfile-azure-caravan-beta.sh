...
get_project_dot_env() {
  ...
  echo "CLIENT_ID" $(jq -r .clientId azure_credentials.json)
  echo "CLIENT_SECRET" $(jq -r .clientSecret azure_credentials.json)
  echo "TENANT_ID" $(jq -r .tenantId azure_credentials.json)
