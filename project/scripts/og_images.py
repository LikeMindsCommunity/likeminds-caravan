...
from utility.firebase import upload_community_thumbnail,upload_tag_thumbnail
...
def  get_all_tags_images():
    ...
    try:
        connection = get_connection()
        curr = connection.cursor()
        sql = """select id,image_link from togther_tags_lpig where image_link!='' order by id desc"""
        curr.execute(sql)
        res = curr.fetchall()
        curr.close()
        connection.close()
...
def upload_all_tags_thumbnail():
    ...
    for file in files:
        count += 1
        upload_tag_thumbnail(file[0],file[1])

        print("thumbnail uploaded for tag=", file[0])
        print("\n")
        if count == 200:
            count=0
            print("\n Process sleep for 5 sec")
