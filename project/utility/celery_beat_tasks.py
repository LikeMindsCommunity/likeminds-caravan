...
class CeleryBeatTask:
    ...
    def get_or_create_new_beat_task(self, *args, **kwargs):
        ...
        if 'interval' in kwargs and kwargs['interval']:
            interval, created = self.get_or_create_interval() if 'interval' in kwargs and kwargs[
                'interval'] else None, False
        ...
        if 'crontab' in kwargs and kwargs['crontab']:
            crontab, created = self.get_or_create_crontab(minute=str(result.tm_min),
                                                          ...
                                                          month_of_year=str(result.tm_mon)) if 'crontab' in kwargs and \
                                                                                               kwargs[
                                                                                                   'crontab'] else None, False
    ...
    def create_dynamic_clery_task(self, args, kwargs, task_name, task_path,
        ...
        if task_name and task_path:
            periodic_task, created = self.get_or_create_new_beat_task(args=args, task_name=task_name,
                                                                      task_path=task_path,
                                                                      date_time=date_time, interval=interval,
                                                                      crontab=crontab,
                                                                      kwargs=kwargs)
