...
def get_general_records(community, day=0):
    ...
    # get chatroomn data
    intro_room = Collabcard.objects.filter(date_epoch__lte=day_1, date_epoch__gte=day_2).filter(
        community__id=community_id).filter(type=1)
    all_rooms = Collabcard.objects.filter(date_epoch__lte=day_1, date_epoch__gte=day_2).filter(
...
def get_weekly_records(community, day=1):
    ...
    # get chatroomn data
    intro_room = Collabcard.objects.filter(date_epoch__lte=day_2, date_epoch__gte=day_1).filter(
        community__id=community_id).filter(type=1)
    all_rooms = Collabcard.objects.filter(date_epoch__lte=day_2, date_epoch__gte=day_1).filter(
        community__id=community_id)
    room_by_cm = 0
    # for c_id in community_id:
    admins = Members.objects.filter(community_id=community_id, state=1)
