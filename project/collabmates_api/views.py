...
def chatroom_delete(request):
    ...
    try:
        ...
        create_chatroom_delete_backup(collabcard_instance, current_user_instance, tag_id, reason, card_creator=is_card_creator,
...
def fetch_deleted_chatroom(request):
    ...
    if request.method == 'GET':
        ...
        deleted_chatrooms = CollabcardStateBackup.objects.select_related('card', 'card__tag').filter(user=user_instance).filter(seen_status=False)
    ...
    if not aj_expired and not status and not state_filter.exists():
        ...
        if guest_header:
            ...
            func_dict = {'collabcard_id': card_instance.id, 'member_id': current_user_id, 'status': True,
                         'is_guest': True, 'source_id': source_id}
            collabcard_follow_internal(func_dict, state=collabcard_states.COLLABCARD_STATE_SEEN)
...
def collabcard_follow(request, function_dict=None):
    ...
    if aj and source_id and (member_state['state'] == 0 or member_state['state'] == member_states.PENDING_MEMBER):
        context = {}
        context = adding_guest_in_chatroom(request, context, collabcard, aj, source_id, community_instance.id,
                                           current_member_id, guest_header=True)
...
def community_collabcard_id(request):
    ...
    collabcard_ids_list = list(
        Collabcard.objects.filter(community=community_instance).filter(~Q(type=4)).order_by('id').values_list('id',
                                                                                                              flat=True))
