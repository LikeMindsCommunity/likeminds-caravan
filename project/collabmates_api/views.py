...
##version 1 api for creating chatrooms

@csrf_exempt
def create_collabcard_version_1(request):
    ''' function to create a card '''
    ...
    is_member = Members.objects.filter(community_id=community_id,member_id=member_id).filter(Q(state=member_states.ADMIN)|Q(state=member_states.MEMBER))
    if not is_member:
        context = get_error_context(False,"You cannot create a chatroom")
        return JsonResponse(context)

    card_instance = create_card_internal_version_1(member_id, community_id, res)


    try:
        context = {'member_id': member_id, 'current_user_id': member_id, 'state_instance': None}
        chatroom = GetChatroomInstanceSerializer(card_instance,context,many=False)
        chatroom = chatroom.data
    except Exception as e:
        context = get_error_context(False,e.args)
        return JsonResponse(context)

    return JsonResponse({'collabcard':chatroom})

def create_card_internal_version_1(user_id, community_id, res):

    '''function to return the collabcard in version 1 api'''
