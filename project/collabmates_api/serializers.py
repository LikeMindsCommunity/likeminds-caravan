...
def CommunitySerializer(community, promoter_id=0, is_owner=False,
                        current_user_id=None, current_user_instance=None, platform_code=None, version_code=None):
    ...
    if promoter_id or is_owner:
        ...
        aj = private_link = generate_private_link(community_instance=community, promoter_instance=current_user_instance,
                                                  just_send_aj=True)
        branch_links = create_community_branch_links(community.id, current_user_id, platform_code, version_code, aj)
    else:
        # only public link
        branch_links = create_community_branch_links(community.id, current_user_id, platform_code, version_code)
...
def UserinfoSerializer(user, sdk_client_info_flag: bool = False):
    ...
    if sdk_client_info_flag:
        ...
        user_meta = get_users_sdk_meta_dict([userinfo['id']], only_sdk_client_info=True)

        userinfo['sdk_client_info'] = user_meta.get(userinfo['id'])
...
def CollabcardSerializer(card, user, community=None, current_user_id=None, preview=False, return_topic=False, 
                         sdk_client_info_flag: bool = False, should_convert_timestamps_to_long: bool = False):
    # function to serialize a community object
    collabcard = {
        'id': card.id,
        'title': card.title,
    ...
    if should_convert_timestamps_to_long:
...
def MembersSerializer(member_instance, community_id, current_user_id=None, send_profile=True,
    ...
    if sdk_client_info_flag:
        ...
        user_meta = get_users_sdk_meta_dict([member_id], only_sdk_client_info=True)
        community_profile['sdk_client_info'] = user_meta.get(member_id)
