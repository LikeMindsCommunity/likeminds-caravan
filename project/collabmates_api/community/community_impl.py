...
from utility.constants import (PLATFORM_CODE_WEB, COMMUNITY_CONFIGURATIONS, MEDIA_LIMITS_CONFIGURATION,
                               ...
                               CHATBOT_DEFAULT_THREAD_CONTEXT, VALID_NOTIFICATION_FEED_ACTIVITIES)
...
class CommunityHelper:
    ...
    def validate_update_community_configurations_request(user_id, api_key, req_body):
        ...
        if configuration_type == NSFW_FILTERING_CONFIGURATION:
            ...
            if update_values.get('notification_feed_activities'): 
                if not isinstance(update_values.get('notification_feed_activities'), dict):
                    return ResponseUtilities.get_inner_error_context("Invalid notification_feed_activities value")    
                
                for key, value in update_values.get('notification_feed_activities').items():
                    
                    if not isinstance(value, bool):
                        return ResponseUtilities.get_inner_error_context(f"Invalid value for key - {key}")
                    
                    if key not in VALID_NOTIFICATION_FEED_ACTIVITIES:
                        return ResponseUtilities.get_inner_error_context(f"Invalid key sent in notification_feed_activities - {key}")
    ...
    def update_configuration_of_community(community_id: int, user_id: int, configuration_type: str, update_values: dict):
        ...
        if configuration_type == MEDIA_LIMITS_CONFIGURATION:
            ...
            if update_values.get('notification_feed_activities') and isinstance(update_values.get('notification_feed_activities'), dict):
                
                if not (configuration_value.get('notification_feed_activities') or isinstance(configuration_value.get('notification_feed_activities'), dict)):
                    configuration_value['notification_feed_activities'] = {}
                ...
                for key, value in update_values.get('notification_feed_activities').items():
                    ...
                    configuration_value['notification_feed_activities'][key] = value
                    record_updated = True
