...
class MemberDirectorySearchView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        member_states = StringUtilities.get_list_from_string(request.GET.get('member_states', None)) 
        ...
        search_manager = SearchImpl(member_id=member_id, search_term=search_term, search_field=search_field, order_by=order_by,
                                    member_states=member_states, follow_status=True, page=page, page_size=page_size, community_id=community_id,
                                    api_key=api_key)
