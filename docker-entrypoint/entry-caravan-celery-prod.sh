...
cd ./project
DJANGO_SETTINGS_MODULE=project.settings.production celery -A project worker --loglevel=info -Q celery -c 4 --without-mingle --without-gossip --without-heartbeat --prefetch-multiplier=1
